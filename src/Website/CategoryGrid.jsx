...
const categories = [
  ...
  const CategoryGrid = ({eventList}) => {
   console.log(eventList)
    return (
      <div className="">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="grid grid-cols-2 gap-4">
            {eventList?.slice(0, 2).map((cat, idx) => (
          
              <div
                key={idx}
                className="relative md:h-full h-52 rounded overflow-hidden shadow-lg"
                style={{
                  background: `linear-gradient(to top, rgba(0,0,0,0.6), transparent), url(${cat.filePath}) center/cover no-repeat`,
                }}
              >
